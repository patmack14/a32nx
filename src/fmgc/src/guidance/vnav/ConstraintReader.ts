...
export class ConstraintReader {
    ...
    extract(geometry: Geometry, activeLegIndex: number) {
        const oldFlightPlanDistance = this.totalFlightPlanDistance;
        ...
        if (VnavConfig.DEBUG_PROFILE && Math.abs(oldFlightPlanDistance - this.totalFlightPlanDistance) > 5) {
            console.warn(`[FMS/VNAV] Big change in flight plan distance encountered (${oldFlightPlanDistance.toFixed(2)} -> ${this.totalFlightPlanDistance.toFixed(2)})`);
        }
    }
