...
export class PFDComponent extends DisplayComponent<PFDProps> {
    ...
    private radioAltitudeSub = Subject.create(new Arinc429Word(0));
    ...
    public onAfterRender(node: VNode): void {
        ...
        sub.on('radioAltitude1Ar').handle((ra) => {
            if (getDisplayIndex() === 1) {
                this.ownRadioAltitude = ra;
            } else {
                this.oppRadioAltitude = ra;
            }
        });

        sub.on('radioAltitude2Ar').handle((ra) => {
    ...
    private determineAndFilterRadioAltitude() {
        ...
        const filteredRadioAltitude = this.radioAltitudeFilter.step(chosenRadioAltitude.value, this.props.instrument.deltaTime / 1000);

        this.filteredRadioAltitude.set(filteredRadioAltitude);
        this.radioAltitudeSub.set(chosenRadioAltitude);
    }
            ...
            >
                <svg class="pfd-svg" version="1.1" viewBox="0 0 158.75 158.75" xmlns="http://www.w3.org/2000/svg">
                    <Horizon
                        ...
                        radioAlt={this.radioAltitudeSub}
